...
namespace our
{
    ...
    void ForwardRenderer::render(World *world)
    {
        ...
        for (auto opaqueCommand : opaqueCommands)
        {
            ...
            glm::mat4 model_view_projection = VP * opaqueCommand.localToWorld;
            opaqueCommand.material->shader->set("transform", model_view_projection);
        ...
        if (this->skyMaterial)
        {
            ...
            // since opengl uses column major matrix we change the last row
        ...
        for (auto transparentCommand : transparentCommands)
        {
            ...
            glm::mat4 model_view_projection = VP * transparentCommand.localToWorld;
            transparentCommand.material->shader->set("transform", model_view_projection);
        ...
        if (postprocessMaterial)
        {
            ...
            // return to default so we can draw the postprocessing pictures
            glBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0);
            ...
            // colorTarget->bind();
            // draw the frame after postprocessing
